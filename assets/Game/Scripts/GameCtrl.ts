...
import { FadeTransition } from './Fadetransicion';
...
gameOver() {
    ...
    Results.scoreFinal = this.result.currentScore;
    Results.bestScore = Math.max(Results.bestScore, this.result.currentScore);

    this.isOver = true;
    this.clip.onAudioQueue(3);

    const canvas = this.node.scene.getChildByName("GameCtrl");
    const fade = canvas.getChildByName("Fade");

    if (fade) {
        const transicion = fade.getComponent(FadeTransition);

        if (transicion) {
            transicion.cambiarEscena("gameover");
        } else {
